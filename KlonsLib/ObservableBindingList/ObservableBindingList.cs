...
namespace KGySoft.ComponentModel
{
        ...
        IBindingList, ICancelAddNew, IRaiseItemChangedEvents
    {
        ...
        /// </summary>
        public ObservableBindingList() : this(new BindingList<T>())
        {
        }
        ...
        public T AddNew()
        {
            if (disposed)
                Throw.ObjectDisposedException();
            if (!AllowNew)
                Throw.InvalidOperationException("AddNes is disabled");
            ...
            if (!canAddNew)
                Throw.InvalidOperationException("AddNew is not supported");
        ...
        {
            if (Items.IsReadOnly)
                Throw.NotSupportedException("Item Move not supported in read only collection");
            if (oldIndex < 0 || oldIndex >= Count)
                Throw.ArgumentOutOfRangeException("oldIndex");
            if (newIndex < 0 || newIndex >= Count)
                Throw.ArgumentOutOfRangeException("newIndex");
        ...
        protected override void RemoveItem(int index)
        {
            ...
            if (!allowRemove && !(addNewPos >= 0 && addNewPos == index))
                Throw.InvalidOperationException("RemoveItem is disabled");
        ...
        protected void CheckReentrancy()
        {
            if (monitor.Busy)
            {
                ...
                if ((collectionChangedHandler?.GetInvocationList().Length ?? 0) + (listChangedHandler?.GetInvocationList().Length ?? 0) > 1)
                    Throw.InvalidOperationException("Reentrancy Not Allowed");
            }
        }
