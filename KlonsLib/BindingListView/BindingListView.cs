...
namespace Equin.ApplicationFramework
{
    ...
    public class BindingListView<T> : AggregateBindingListView<T>
    {
            ...
            set
            {
                ...
                if (!(value is ICollection<T>))
                {
                    ...
                    foreach (object item in value)
                    {
                        if (!(item is T))
                        {
                            throw new ArgumentException(string.Format(Messages.InvalidListItemType, typeof(T).FullName), "DataSource");
                        }
                    }
                }
        ...
        public override string ToString() => $"BindingListView({typeof(T).Name})";
